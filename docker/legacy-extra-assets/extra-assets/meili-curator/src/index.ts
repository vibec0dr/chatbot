...
function pickDateFromDoc(doc: any): string | null {
  for (const key of ['updatedAt', 'updated_at', 'updated', '_updatedAt', 'lastUpdated', 'modifiedAt', 'modified_at']) {
    if (doc[key]) return doc[key];
  }
  if (doc.updated_at_ms && typeof doc.updated_at_ms === 'number') return new Date(doc.updated_at_ms).toISOString();
  return null;
}

async function fetchDocumentsPage(host: string, index: string, offset = 0, limit = 1000, apiKey?: string) {
...
async function deleteIds(host: string, index: string, ids: Array<string | number>, apiKey?: string) {
...
export async function runCurator(options: CLIOptions) {
  const { host, apiKey, index, days, batchSize, dryRun } = options;
  const threshold = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
  console.log(`Curating index="${index}" (threshold ${threshold.toISOString()})`);
  let offset = 0;
  let totalDeleted = 0;
  let totalSeen = 0;

  while (true) {
    const docs: any[] = await fetchDocumentsPage(host, index, offset, batchSize, apiKey);
    ...
    if (docs.length < batchSize) break; // done
    offset += docs.length;
  }
...
async function main(argv: string[]) {
  ...
  if (values.help || !values.index) {
    console.error('Usage: meili-curator --index <name> --days <days> [--host <url>] [--key <apiKey>] [--batch <n>] [--dryRun]');
    process.exit(values.help ? 0 : 2);
  }

  const host = (values.host as string) ?? process.env.MEILI_HOST ?? 'http://localhost:7700';
  const apiKey = (values.key as string) ?? process.env.MEILI_MASTER_KEY;
  const index = (values.index as string);
  ...
  await runCurator({ host, apiKey, index, days, batchSize, dryRun });
}
