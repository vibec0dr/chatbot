...
async function fetchAndInsertFromMongo(dbClient, collectionName, tempIndexName, start, end) {
    const batchSize = 500;
...
async function run() {
    ...
    try {
        ...
        await fetchAndInsertFromMongo(mongoClient, "conversations", "conversations_temp", start, end);
        await fetchAndInsertFromMongo(mongoClient, "messages", "messages_temp", start, end);
        await swapIndexes();
    }
    finally {
